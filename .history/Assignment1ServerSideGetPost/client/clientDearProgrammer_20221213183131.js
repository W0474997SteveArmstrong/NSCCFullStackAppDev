  ...
  fetchAPIAfterStatusCodeAndBooleanCheck.then((json) => {
    ...
    for(const genre of json)
    {
      console.log('genre is',genre);
        ...
        <td>
        <a href="#" id="edit" class="btn btn-warning btn-sm edit">Edit</a>
        </td>
        <td>
        <a href="#"  id="del"class="btn btn-danger btn-sm-delete">Delete</a>
        </td>
        </tr>
        `;
    }
...
document.getElementById(".btn btn-warning btn-sm edit").addEventListener("click",(e)=>{
  target = e.target;
  console.log('target is',target);
  if(target.classList.contains("delete")){
target.parentElement.parentElement.remove();
    alert('The Genre has been deleted as per your request');
  }
