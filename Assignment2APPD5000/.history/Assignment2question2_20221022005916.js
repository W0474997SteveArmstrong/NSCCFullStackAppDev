...
//takes story_file as input
function loopingLineByLine(lineArr) {
  for (const storyFileLineArr of lineArr) {
    console.log("Story file line Array is:", storyFileLineArr);
    // Row is: an animal name,aardvark,hippo,duck,pelican,t-rex
    //Now we need to split the row based on comma
    Arr2DStoryFile.push(storyFileLineArr.split(DELIMITER));
    console.log("The InnerRowArray is: ", Arr2DStoryFile);
    console.log("The  is: ", Arr2DStoryFile);
    ...
    let counter=0;
    let totalWordRowArr =[];
    for (let innerRow of Arr2DStoryFile) {
        //  Inner row is:
        //   [ 'an animal name', 'aardvark', 'hippo', 'duck', 'pelican', 't-rex' ]
    //   ]
      ...
      for (let rowStoryFile of fileRead("files/the_story_file.txt", "utf-8")) {
        ...
        let wordRow = rowStoryFile.split(" ");
         totalWordRowArr.push(wordRow);
         console.log("TotalWordRow Arr is: ",totalWordRowArr);
...
//function loopinglinebyline exit
//Method Overloading
