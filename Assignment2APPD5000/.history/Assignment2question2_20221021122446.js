...
let rowArray = [];
let userInput = null;
let activdescribeWordTypity=null;
let usrInp = function getUserInput(describeWordTyp) {
    userInput = RLS.question("Please choose "+describeWordTyp+":");
    while (0==userInput) {
...
for (const row of lineArr) {
    ...
    for (const innerRow of rowArray){
       describeWordTyp =  innerRow[0];
        innerRow.slice(1);
       console.log("Activity is:",describeWordTyp);
       usrInp(describeWordTyp);
    }
}
