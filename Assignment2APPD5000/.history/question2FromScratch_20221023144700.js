...
function loopingLineByLineChoices(lineArrChoice) {
  for (const rowChoice of lineArrChoice) {
    ...
    let counter=0;
    for (let innerRowChoice of innerRowChoiceArray) {
      phrase = innerRowChoice[0];
      wordchoice = innerRowChoice.slice(1);
...
loopingLineByLineChoices(
  fileReadChoices("files/the_choices_file.csv", "utf-8")
);
