...
let usrInp = function getUserInput() {
  ...
  for (const row of lineArr) {
    ...
    for (let innerRow of innerRowArray) {
      ...
      fileRead=null;
      let storyFileRead=null;
      let storyFileRead = fileRead("files/the_story_file.txt", "utf-8");
      console.log("StoryFileRead is:",storyFileRead);
      if(userInputArrTotal.includes(A)){
        for(const rowStoryFile of storyFileRead){
