...
fetch(`https://restcountries.com/v2/all`)
    ...
    function getPopulation(json){
        json.filter(country=>country.borders) //grab only countries which have border
        .filter(country=>country.borders.includes("CHN"))//after grabbing countries which have border,grab only those countries which border China
        .filter(country=>country.languages.filter((language)=>language.name=="English").length==1) //grab only those China bordering countries where language is 
        //a property and only select those countries which have English as the official language
        .reduce((populationCount,country)=>(populationCount+=country.population)); //get the count of their population
