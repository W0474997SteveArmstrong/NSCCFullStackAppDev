...
for (let index = 0; index < myGridsize; index++) {
  ...
  while (null == x) {
    ...
    while (x >= myGridsize) {
      x = RLS.question(
        "Please enter a valid x co-ordinate which must be less than your chosen grid size of " +
          myGridsize +": "
      );
    }
  }
  ...
  while (null == y) {
    ...
    while (y >= myGridsize) {
      y = RLS.question(
        "Please enter a valid y co-ordinate which must be less or equal to than your chosen grid size of " +
          myGridsize +": "
      );
    }
  }
