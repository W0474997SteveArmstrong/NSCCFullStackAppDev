...
function makeHeaders(size){3
    ...
    let doublespacebtwnRowIndexAndColHeader ='  ';
    let result =doublespacebtwnRowIndexAndColHeader;
    let singlespacebtwnColHeader=' ';
    for (let index = 0; index < size; index++) {
        result+=index+singlespacebtwnColHeader;
    }
    return result;
}
...
function printGrid(gridArr,isOpponent){
    ...
    for (let rowIndex = 0; rowIndex < gridArr.length; rowIndex++) {
            let singleSpaceBtwnRowIndexAndColValue =
            let gridRow = rowIndex + ' ';
