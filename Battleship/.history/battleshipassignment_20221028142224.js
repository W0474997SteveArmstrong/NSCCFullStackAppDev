...
function makeGrid(){
    ...
    let userInputArr=[];
    for (let rowIndex = 1; rowIndex < (fileContentArr.length); rowIndex++) {
        /*At every index of the outer grid, an empty grid is created unitl 
        index=size-1 of the grid.*/
        userInputArr[rowIndex]=[];
        for (let colIndex = 1; colIndex <(fileContentArr.length); colIndex++) {
...
function printGrid(userInputArr){
    ...
    console.log(header);
    for (let rowIndex = 1; rowIndex < userInputArr.length; rowIndex++) {
