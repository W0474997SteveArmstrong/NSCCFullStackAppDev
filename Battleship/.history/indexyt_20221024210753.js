...
while(null==myGridsize){
    myGridsize = parseIntRLS.question("What size do you want your grid to have?: ");
    while(0===myGridsize){
        myGridsize=RLS.question("Please enter a non-zero grid size: ");
    }
}
while(null==opponentGridsize){
    opponentGridsize=RLS.question("What size grid would you want your opponent to have?: ");
    while(0===opponentGridsize){
        opponentGridsize=RLS.question("Please enter a non-zero opponent grid size: ");
    }
}
...
for (let index = 0; index < myGridsize; index++) {
  ...
  while (null == x) {
    ...
    if (x > myGridsize) {
      x = RLS.question(
        "Please enter a valid co-ordinate which must be less than your chosen grid size of: " +
          myGridsize
      );
    }
  }
